...
RDBMS(Relational Database System) is a system that manages relational databases.
SQL(Structured Query Language) is what we use to interact with the datatbase, managing the databases and the data within them
1. Why do tables need a `primary key`? We something that uniquely identify each row.
